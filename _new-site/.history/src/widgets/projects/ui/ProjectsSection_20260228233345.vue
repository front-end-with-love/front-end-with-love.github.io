...
// Кривая плавности для скролла (ease-in-out cubic)
function easeInOutCubic(t: number): number {
  return t < 0.5 ? 4 * t * t * t : 1 - (-2 * t + 2) ** 3 / 2
}

// Плавно проскроллить к элементу (rAF + easeInOutCubic), по завершении вызвать onEnd
function scrollToElementThen(el: HTMLElement, onEnd: () => void) {
  const remPx = parseFloat(getComputedStyle(document.documentElement).fontSize) || 16
  const targetY = el.getBoundingClientRect().top + window.scrollY - TOP_OFFSET_REM * remPx
