...
function updatePaginationVisible() {
  const sliderWrap = sectionRef.value?.querySelector('.workflow__slider-wrap') as HTMLElement | null
  if (!sliderWrap) return
  const vh = window.innerHeight
  const { top, bottom } = sliderWrap.getBoundingClientRect()
  paginationVisible.value = top <= vh * 0.85 && bottom < vh * 0.85
}

onMounted(() => {
  updatePaginationVisible()
  window.addEventListener('scroll', updatePaginationVisible, { passive: true })
  window.addEventListener('resize', updatePaginationVisible)
})

onUnmounted(() => {
  window.removeEventListener('scroll', updatePaginationVisible)
